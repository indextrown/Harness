...
- PR 제목은 최종 squash 병합 커밋 제목의 기반이 되므로 `type: subject` 또는 `type(scope): subject` 형식을 사용합니다.
- PR 제목 끝에 `(#번호)`는 직접 붙이지 않고 squash 병합 시 자동으로 붙입니다.
브랜치 정리 규칙:
...
- 정상적인 병합 경로에서는 작업 브랜치마다 PR을 반드시 먼저 생성합니다.
- PR 병합은 항상 squash만 사용합니다.
- 최종 병합 커밋 제목은 `type: subject (#PR번호)` 형식을 사용합니다.
- PR 없이 `main`에 직접 병합하는 로컬 merge는 금지하며, 예외 복구 상황에서만 제한적으로 허용합니다.
...
- 일반 커밋은 `type: subject` 또는 `type(scope): subject` 형식을 사용합니다.
- 기본 타입은 `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `style`, `perf`, `ci`, `build`를 사용합니다.
- 제품 동작 추가는 `feat`, 버그 수정은 `fix`, 하네스/설정/운영 변경은 `chore`, 문서 변경은 `docs`를 우선 사용합니다.
- 제목은 짧은 명령형으로 적고 불필요한 마침표를 붙이지 않습니다.
- 자동 생성되는 병합 커밋과 `Revert` 커밋은 예외로 둡니다.
